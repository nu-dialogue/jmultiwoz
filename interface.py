INTERFACE_HTML = """
<html>
    <link rel="stylesheet" href={style_sheet_href} />
    <style>
    .balloon1-left {{
      position: relative;
    ...
    <body>
        ...
        <script>
            ...
            const maxTurn = {max_turn};
            document.getElementById("turn").innerHTML = maxTurn;
            document.getElementById("turn-half").innerHTML = Math.trunc(maxTurn / 2);
            ...
            document.getElementById("interact").addEventListener("submit", function(event){{
                ...
                parDiv.scrollTo(0, parDiv.scrollHeight);
                var send_info = {{"session_id": sessionId, "user_input": ntex}};
            ...
            function sendUtterance(send_info) {{
                fetch('/interact', {{
                    ...
                    // myContext.push({{"spk": "[SPK2]", "utt": send_info["user_input"]}});
            ...
            function createEvalForm() {{
                var parDiv = document.getElementById("parent");
                var questionList = [
                    "1: 対話を通して、ボットの対話の流れはスムーズだった",
                    "2: 対話を通して、ボットは十分な情報を提示していた",
                    "3: 対話を通して、自分は対話に満足した"
                ]
                ...
                parDiv.append(createChatRow("System", "全ての設問に回答したら、下の「評価終了」ボタンをクリックしてください。"));
                parDiv.scrollTo(0, parDiv.scrollHeight);
            }}
